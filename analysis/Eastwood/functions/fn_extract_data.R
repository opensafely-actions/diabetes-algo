...
fn_extract_data <- function(input_filename) {
  ...
  data_extract <- data_extract %>%
    mutate(
      ...
      ), # rounding down the date to the nearest day
      across(contains('birth_year'), ~ format(as.Date(.), "%Y")), # specifically for birth_year, then pass it onto _num to reformat birth_year_num
